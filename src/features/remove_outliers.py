...
df[outlier_columns[:3] + ["label"]].boxplot(
    by="label", figsize=(20, 10), grid=True, layout=(1, 3)
)
df[outlier_columns[3:] + ["label"]].boxplot(
    by="label", figsize=(20, 10), grid=True, layout=(1, 3)
)
...
df[outlier_columns[:3] + ["label"]].plot.hist(
    by="label", figsize=(20, 10), grid=True, layout=(3, 3)
)
df[outlier_columns[3:] + ["label"]].plot.hist(
    by="label", figsize=(20, 10), grid=True, layout=(3, 3)
)
...
# Loop over all columns
dataset, outliers, X_scores = mark_outliers_lof(df, outlier_columns, n=20)
for col in outlier_columns:
    plot_binary_outliers(
...
dataset, outliers, X_scores = mark_outliers_lof(
    df[df["label"] == label], outlier_columns, n=20
)
for col in outlier_columns:
    plot_binary_outliers(
        dataset=dataset, col=col, outlier_col="outlier_lof", reset_index=True
...
dataset[dataset["gyro_x_outlier"]]
dataset.loc[dataset["gyro_x_outlier"], "gyro_x_outlier"] = np.nan
# Create a loop
...
for col in outlier_columns:
    for label in df["label"].unique():
        ...
        removed_outliers = len(dataset) - len(dataset[col].dropna())
        print(
            f"Removed {removed_outliers} outliers from column {col} for label {label}"
        )
